...
import {Blog, BlogViewPageComponentProps} from "@/type";
...
const BlogViewPageComponent: FC<BlogViewPageComponentProps> = ({blog}) => {
    ...
    const [data] = useState<Blog>(blog)
    useBodyClass('bg-white');

    return (
        <section className='w-full flex flex-col md:flex-row'>
            <div className='w-full md:w-[70%]'>
                <ViewBlogComponent isLogin={isLogin} data={data}/>
            </div>
                    ...
                    <div className=" w-[90%] bg-white rounded-xl shadow-md  overflow-hidden max-w-sm mx-auto my-5">
                        ...
                        <div className="px-6 py-4 text-center">
                            ...
                            <div className="mt-4 text-left text-sm text-gray-800">
                                ...
                                <p className="mt-2"><span className="font-bold">STACK</span><br/>{data?.tags.join(',')}
                                </p>
                                <p className="mt-2"><span
                                    className="font-bold">INICIÉ</span><br/>{format(data.user.start, "d 'de' MMMM yyyy", {locale: es})}
                                </p>
                            </div>
                        </div>
                        <Link
